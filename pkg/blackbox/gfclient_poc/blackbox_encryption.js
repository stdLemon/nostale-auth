...
function decrypt(encrypted_blackbox, gs_id, account_id) {
    return xor(Buffer.from(encrypted_blackbox, "base64"), create_key(gs_id, account_id)).toString();
}

function encrypt(blackbox, gs_id, account_id) {
    return xor(Buffer.from(blackbox), create_key(gs_id, account_id)).toString("base64");
}
